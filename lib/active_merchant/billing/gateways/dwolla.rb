    ...
    class DwollaGateway < Gateway
      class DwollaPostData < PostData
        def to_json_post_data
          ...
          %-{"Key":"#{self[:key]}","Secret":"#{self[:secret]}","AllowFundingSources":"true",#{urls}#{test_string}"PurchaseOrder":{"DestinationId":"#{purchase_order[:destination_id]}","Discount": #{"%.2f" %  purchase_order[:discount]},"OrderItems":[#{items.join(',')}],"Shipping": #{"%.2f" % purchase_order[:shipping]},"Tax": #{"%.2f" % purchase_order[:tax]},"Total": #{"%.2f" % purchase_order[:total]},"FacilitatorAmount": #{"%.2f" % purchase_order[:facilitator_amount]}}}-
       end
      end
